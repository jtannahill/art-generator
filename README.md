...
EventBridge (daily 06:00 UTC)
    └── Step Function (concurrency 2)
        ...
        └── Site Rebuild (asset mirror)
            └── Mirrors new weather/ + palettes/ files into the site/ prefix
```

The static HTML frontend now lives in the separate [art-astro](https://github.com/jtannahill/art-astro) repo. That repo also owns sitemap.xml (via `@astrojs/sitemap`), robots.txt, and llms.txt (served from its `public/` directory). The Jinja2 renderer in `art-site-rebuild` is retired; the Lambda survives only as an S3 asset mirror.
...
- **Open Graph + Twitter Cards**: unique title, description, 1200px WebP preview image per artwork page
...
- **robots.txt**: allows all crawlers, sitemap reference (served from art-astro `public/`)
- **sitemap.xml**: generated by `@astrojs/sitemap` in the art-astro repo, all pages + all artwork
- **llms.txt**: structured for AI crawlers (ChatGPT, Claude, Perplexity), maintained in art-astro `public/`
